    ...
    <div class="narrowContent">
        <div class="profileBox" style="width: 500px;">
            <div class="twitterProfile">
                ...
                <div class="bio">
                    <p>@<a href="http://twitter.com/<%= Model.Username %>" class="twitterLink"><%= Model.Username %></a> has <%= Model.CurrentPointsTotalFormatted %> points (<a href="http://twitter.com/?status=%40HelloApp+met+%40<%= Model.Username %>">I met @<%= Model.Username %></a>)</p>
                    <p id="latestTweet"></p>
                </div>
            </div>                        
            <ul class="categories">
            ...
            <% foreach (Token t in ViewData["redeemedTokens"] as List<Token>) { %>
                <img class="tokenImage" src="/Content/images/tokens/<%= t.FileName %>" alt="<%= t.Text %> (<%=t.Campaign.Value %> points)" title="<%= t.Text %> (<%=t.Campaign.Value %> points)" />
            <% } %>
            </div>
            <p class="tagPara">
